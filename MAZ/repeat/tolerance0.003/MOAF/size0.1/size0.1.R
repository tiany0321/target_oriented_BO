setwd("C:\\Users\\12758\\Desktop\\target_oriented_BO\\MAZ\\repeat\\tolerance0.003\\MOAF\\size0.1")
start_time<-Sys.time()
library(rPref)
library(dplyr)
library("DiceKriging")
...
fn.utility=function(data.training,estimator,training.data,infj,tar_num){
  ...
  #LCB
  if (estimator==3){ 
    fn.LCB = function(training.data,infj,f=0.01,delta=0.1)
    {
      LCB = infj[,"mean"] -(2*f*log10(nrow(training.data)*(nrow(training.data)-size.i+1)^2*3.14^2/(6*delta)))^0.5*infj[,"sd"]
      #  LCB = infj[,"mean"] -1*infj[,"sd"]
      LCB = data.frame(LCB)
      return (LCB)
    }
    LCB.v= fn.LCB(training.data,infj,f=0.01,delta=0.1)################parameter delta
  ...
  #EQI
  if (estimator==4){ 
    fn.EQI.ei = function(training.data,infj,eqi.beta=0.85,epsilon=(10^(-5))^0.5)
    {
  ...
  if (estimator==5){ 
    ...
    M_score=as.data.frame(scale(M_score))
    # 计算 Pareto 前沿（目标是最大化）
...
iter_total=matrix(,1,10)
iter_mean=matrix(0,train.num,1)
iter_mean_w=matrix(0,train.num,1)
ave.OC=matrix(,nrow(iter)-1,10)
ave.dis=matrix(,nrow(iter)-1,10)
ave.OC_total=matrix(,nrow(iter)-1,10)
ave.dis_total=matrix(,nrow(iter)-1,10)
...
repeat{
  ...
  for(size in size.i)
  {
    ...
    for(i in 1:train.num)
    {
      ...
      if(min(abs(training.data[,"es"]-data.training[tar_num,"es"]))<=0.003*(y_range))
      {
        ...
        repeat{
          ...
          if(sum(EI)==0||(nrow(unique(EI))==1&nrow(EI)>1))
          {
            ...
            if(min(abs(training.data0[,"es"]-data.training[tar_num,"es"]))<=0.003*(y_range))
            {
              if(u>=nrow(iter)-2){
                break
              }else{
...
colnames(ave.dis)=c("ego","ego.sd","PI","PI.sd","LCB","LCB.sd","EQI","EQI.sd","MOAF","MOAF.sd")
write.csv(ave.dis,"ave.dis.csv")
colnames(ave.OC)=c("ego","ego.sd","PI","PI.sd","LCB","LCB.sd","EQI","EQI.sd","MOAF","MOAF.sd")
write.csv(ave.OC,"ave.OC.csv")
colnames(iter_total)=c("ego","ego.sd","PI","PI.sd","LCB","LCB.sd","EQI","EQI.sd","MOAF","MOAF.sd")
