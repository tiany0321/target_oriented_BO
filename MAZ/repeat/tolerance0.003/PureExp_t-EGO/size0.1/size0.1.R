setwd("C:\\Users\\12758\\Desktop\\target_oriented_BO\\MAZ\\repeat\\tolerance0.003\\PureExp_t-EGO\\size0.1")
...
repeat{
  ...
  for(size in size.i)
  {
    ...
    for(i in 1:train.num)
    {
      ...
      if(min(abs(training.data[,"es"]-data.training[tar_num,"es"]))<=0.003*(max(data.training[,"es"])-min(data.training[,"es"])))
      {
        ...
        repeat{
          ...
          if(sum(EI)==0||(nrow(unique(EI))==1&nrow(EI)>1))
          {
            ...
            if(min(abs(training.data0[,"es"]-data.training[tar_num,"es"]))<=0.003*(max(data.training[,"es"])-min(data.training[,"es"])))
            {
              if(u>=nrow(iter)-2){
                break
              }else{
...
colnames(ave.dis)=c("pre_value","pre_value.sd","tego","tego.sd")
write.csv(ave.dis,"ave.dis.csv")
colnames(ave.OC)=c("pre_value","pre_value.sd","tego","tego.sd")
write.csv(ave.OC,"ave.OC.csv")
colnames(iter_total)=c("NULL","NULL","pre_value","pre_value.sd","tego","tego.sd","NULL","NULL")
