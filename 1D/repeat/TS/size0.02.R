setwd("C:\\Users\\12758\\Desktop\\target_oriented_BO\\1D\\repeat\\TS")
start_time<-Sys.time()
fn.utility=function(estimator,data.training,training.data.x, training.data.y,virtual.data.x,tar_num, n_samples = 1){
  
  
  if(estimator==1){
    # Thompson Sampling 
    thompson_sampling <- function(data.training,training.data.x, training.data.y,virtual.data.x,tar_num, n_samples = 1) 
    {
      
      set.seed(123)
      noise=rep(10^(-5),nrow(training.data.x))
      m<-  km(formula<-~.,design=training.data.x,noise.var = noise, response=training.data.y)
      sample_function <- simulate(m, nsim = n_samples, newdata = virtual.data.x)
      
      # Find the optimal solution on the sample function
      diff_values = - abs(sample_function - data.training[tar_num,"es"])
      # closest_value_num <- which.min(diff_values)
      
      return(diff_values)
    }
    EI= thompson_sampling(data.training,training.data.x, training.data.y,virtual.data.x,tar_num, n_samples = 1)
    EI=t(as.data.frame(EI))
    EI[which(EI[,]=="NaN"),]=0
    colnames(EI)="EI"
    rownames(EI)=rep(1:nrow(EI))
  }
...
#total.data.y <- as.data.frame(abs(f11_xiong(total.data.x)-f11_xiong(total.data.x)[tar,]))
...
repeat{
  ...
  for(size in size.i)
  {
    ...
    for(i in 1:train.num)
    {
      ...
      EI0= fn.utility(estimator,data.training,training.data.x, training.data.y,virtual.data.x,tar_num, n_samples = 1)
      ...
      if(min(abs(training.data[,"es"]-data.training[tar_num,"es"]))<=0.01*y_range)
      {
        ...
        repeat{
          ...
          EI=fn.utility(estimator,data.training,training.data0.x, training.data0.y,virtual.data0.x,tar_num, n_samples = 1)
          EI=as.data.frame(EI)
...
colnames(ave.dis)=c("TS","TS.sd","pre_value","pre_value.sd","Tego","Tego.sd")
write.csv(ave.dis,"ave.dis.csv")
colnames(ave.OC)=c("TS","TS.sd","pre_value","pre_value.sd","Tego","Tego.sd")
write.csv(ave.OC,"ave.OC.csv")
colnames(iter_total)=c("TS_u","TS_u.sd","pre_value","pre_value.sd","Tego","Tego.sd")
